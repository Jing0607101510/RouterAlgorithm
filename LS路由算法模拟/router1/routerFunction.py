...
def deal_client_input(router):
	while True:
		...
		if cmd == '1':
			...
			print("从网络中断开！")
			router.seq += 1
			router.broadcast_offline(router.local_addr)
...
def deal_router_io(router, connect_socket, addr):
	...
	if tuple(des_addr) == router.local_addr:
		if msg_type == Type.ONLINE:
			router.print_date_and_name()
			if sou_addr not in router.online_routers:
				print("[%s : %s]上线了！"%sou_addr)
				router.deal_online(sou_addr)
		elif msg_type == Type.DATA:
			router.print_date_and_name()
			print("[%s : %s]"%(sou_addr) + "发来消息:", msg)
		elif msg_type == Type.OFFLINE and seq != router.seq:
			lock = threading.Lock()
			lock.acquire()
			router.print_date_and_name()
			...
			lock.release()
		elif msg_type == Type.ACK:
			print("[%s : %s]发来上线确认！"%sou_addr)
			router.deal_ack(sou_addr)
	else:
